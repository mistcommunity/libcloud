...
class AWSRequestSignerAlgorithmV4(AWSRequestSigner):
    ...
    def _get_signature(
        self, params, headers, dt, method, path, data, for_sts_service=False
    ):
    ...
    def _get_key_to_sign_with(self, dt, for_sts_service=False):
        service_name = (
            self.STS_SERVICE_NAME if for_sts_service else self.connection.service_name
        )
        return _sign(
            _sign(
                _sign(
    ...
    def _get_string_to_sign(
        ...
        return "\n".join(
            [
                ...
                self._get_credential_scope(dt, for_sts_service=for_sts_service),
    ...
    def _get_canonical_request(
        self, params, headers, method, path, data, use_canonical_query_string=False
    ):
    ...
    def generate_sts_presigned_url(self, params, host, dt=None):
        ...
        headers = {"host": host, "x-k8s-aws-id": params["headers"]["x-k8s-aws-id"]}
