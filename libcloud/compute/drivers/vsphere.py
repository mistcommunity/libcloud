...
class VSphereNodeDriver(NodeDriver):
    ...
    def ex_remove_snapshot(self, node, snapshot_name=None, remove_children=False):
        """
    ...
    def ex_revert_to_snapshot(self, node, snapshot_name=None):
        ...
        if not vm.snapshot:
            raise LibcloudError(
                "Revert failed. No snapshots " "for node %s" % node.name
            )
        snapshots = recurse_snapshots(vm.snapshot.rootSnapshotList)
        if not snapshot_name:
            snapshot = snapshots[-1].snapshot
    ...
    def create_node(self, **kwargs):
        ...
        if kwargs.get("ex_folder"):
            ...
            if folder is None:
                folder = self._get_item_by_moid("Folder", kwargs.get("ex_folder"))
        else:
            folder = datacenter.vmFolder

        if kwargs.get("resource_pool"):
            resource_pool = self.get_obj(
                [vim.ResourcePool], kwargs.get("resource_pool")
            )
        else:
            try:
        ...
        if kwargs.get("ex_datastore"):
            datastore = self.get_obj([vim.Datastore], kwargs.get("ex_datastore"))
            if datastore is None:
                datastore = self._get_item_by_moid(
                    "Datastore", kwargs.get("ex_datastore")
                )
        elif not datastore:
...
class VSphere_6_7_NodeDriver(NodeDriver):
    ...
    def __init__(
        ...
        super(VSphere_6_7_NodeDriver, self).__init__(
