...
class DigitalOcean_v2_NodeDriver(DigitalOcean_v2_BaseDriver, DigitalOceanNodeDriver):
    ...
    def create_volume(
        self, size, name, filesystem_type=None, location=None, snapshot=None
    ):
        """
        Create a new volume.
    ...
    def ex_list_snapshots(
        ...
        data = self._paginated_request("/v2/snapshots", "snapshots", params=params)
    ...
    def ex_delete_snapshot(self, snapshot: DigitalOceanSnapshot) -> bool:
        ...
        resp = self.connection.request(f"/v2/snapshots/{snapshot.id}", method="DELETE")
    ...
    def _to_image(self, data):
        ...
        return NodeImage(id=data["id"], name=name, driver=self, extra=extra)
