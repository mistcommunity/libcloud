...
class SolusVMNodeDriver(NodeDriver):
    ...
    def __init__(
        self, key=None, secret=None, host="solusvm.com", port=None, verify=True
    ):
        """
        :param key: username
        ...
        super(SolusVMNodeDriver, self).__init__(
        ...
        try:
            ...
            raise Exception(
                "Make sure solusvm host is accessible and port " "%s is open" % port
            )
    ...
    def create_node(
        ...
        for i in range(0, 10):
            ...
            for node in nodes:
                if (
                    node.id not in [n.id for n in existing_nodes]
    ...
    def ex_start_node(self, node):
        ...
        Start a node
        """
        action = "/api/virtual_machines/{identifier}/boot".format(identifier=node.id)
    ...
    def reboot_node(self, node):
        ...
        Reboot a node
        """
        action = "/api/virtual_machines/{identifier}/reboot".format(identifier=node.id)
    ...
    def destroy_node(self, node):
        ...
        Delete a VS
        """
        action = "/api/virtual_machines/{identifier}".format(identifier=node.id)
    ...
    def ex_list_vs_parameters(self, vttype):
        ...
        """
        response = self.connection.request(
            "/api/virtual_machines/" "createvm_params/%s" % vttype
        )
