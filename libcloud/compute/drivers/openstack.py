...
class OpenStackNodeDriver(NodeDriver, OpenStackDriverMixin):
    ...
    def create_volume(
        ...
        resp = self.connection.request(
            "/os-volumes", method="POST", data={"volume": volume}
        )
    ...
    def destroy_volume(self, volume):
        return self.connection.request(
            "/os-volumes/%s" % volume.id, method="DELETE"
        ).success()
    ...
    def get_image(self, image_id):
        ...
        return self._to_image(
            self.connection.request("/images/%s" % (image_id,)).object["image"]
        )
...
class OpenStack_1_0_NodeDriver(OpenStackNodeDriver):
    ...
    def create_node(
        ...
        if ex_shared_ip_group:
            ...
            # name.
            warnings.warn(
                "ex_shared_ip_group argument is deprecated."
        ...
        resp = self.connection.request(
            "/servers", method="POST", data=ET.tostring(server_elm)
        )
    ...
    def ex_resize_node(self, node, size):
        ...
        """
        elm = ET.Element(
            "resize", {"xmlns": self.XML_NAMESPACE, "flavorId": str(size.id)}
        )
    ...
    def ex_create_ip_group(self, group_name, node_id=None):
        ...
        group_elm = ET.Element(
            "sharedIpGroup", {"xmlns": self.XML_NAMESPACE, "name": group_name}
        )
    ...
    def ex_limits(self):
        ...
        rate = [
            _to_rate(el) for el in findall(limits, "rate/limit", self.XML_NAMESPACE)
        ]
        absolute = {}
    ...
    def create_image(self, node, name, description=None, reboot=True):
        ...
        return self._to_image(
            self.connection.request(
                "/images", method="POST", data=ET.tostring(image_elm)
            ).object
        )
    ...
    def _to_shared_ip_group(self, el):
        ...
        if servers_el:
            servers = [
                s.get("id")
                for s in findall(servers_el[0], "server", self.XML_NAMESPACE)
            ]
        else:
            servers = None
        return OpenStack_1_0_SharedIpGroup(
            id=el.get("id"), name=el.get("name"), servers=servers
        )
...
class OpenStackNeutronNetwork(object):
    ...
    def __init__(self, id, name, status, subnets=[], router_external=False, extra={}):
        self.id = id
        self.name = name
        self.status = status
...
class OpenStackSubnet(object):
    ...
    def __repr__(self):
        return "<OpenStackSubnet id=%s name=%s cidr=%s>" % (
            self.id,
            self.name,
            self.cidr,
        )
...
class OpenStackRouter(object):
    ...
    def __repr__(self):
        return "<OpenStackRouter id=%s name=%s external_gateway=%s>" % (
            self.id,
            self.name,
...
class OpenStackKeyPair(object):
    ...
    def __init__(
        self, name, fingerprint, public_key, driver, private_key=None, extra=None
    ):
        """
        Constructor.
...
class OpenStack_1_1_NodeDriver(OpenStackNodeDriver):
    ...
    def _create_args_to_params(self, node, **kwargs):
        ...
        if kwargs.get("ex_blockdevicemappings", None):
            if kwargs["ex_blockdevicemappings"][0].get("volume_id"):
                server_params["block_device_mapping"] = kwargs["ex_blockdevicemappings"]
            else:
                server_params["block_device_mapping_v2"] = kwargs[
    ...
    def _to_subnet(self, obj):
        return OpenStackSubnet(
            ...
            allocation_pools=obj.pop("allocation_pools", []),
            gateway_ip=obj.pop("gateway_ip", ""),
            cidr=obj.pop("cidr", ""),
            ip_version=obj.pop("ip_version", 4),
            extra=obj,
        )
    ...
    def ex_list_subnets(self, filters=None):
        ...
        subnets = self.connection.request(
            self._subnets_url_prefix, params=params
        ).object
    ...
    def ex_delete_network(self, network):
        ...
        """
        response = self.connection.request(
            self._networks_url_prefix + "/%s" % network.id, method="DELETE"
        ).object
    ...
    def ex_delete_subnet(self, subnet_id):
        ...
        response = self.connection.request(
            self._subnets_url_prefix + "/%s" % subnet_id, method="DELETE"
        ).object
    ...
    def ex_open_console(self, node_id):
        ...
        resp = self.connection.request(uri % node_id, method="POST", data=data).object
    ...
    def create_volume_snapshot(
        self, volume, name=None, ex_description=None, ex_force=True
    ):
        """
    ...
    def ex_create_snapshot(self, volume, name, description=None, force=False):
        ...
        return self.create_volume_snapshot(
            volume, name, ex_description=description, ex_force=force
        )
    ...
    def _to_security_group_rules(self, obj):
        return [
            self._to_security_group_rule(security_group_rule)
            for security_group_rule in obj
        ]
    ...
    def _to_security_groups(self, obj):
        ...
        return [
            self._to_security_group(security_group)
            for security_group in security_groups
        ]

    def _to_security_group(self, obj):
        rules = self._to_security_group_rules(
            obj.get("security_group_rules", obj.get("rules", []))
        )
    ...
    def ex_delete_security_group_rule(self, rule):
        ...
        """
        resp = self.connection.request(
            "/os-security-group-rules/%s" % (rule.id), method="DELETE"
        )
        return resp.status == httplib.NO_CONTENT
    ...
    def delete_key_pair(self, key_pair):
        ...
        """
        response = self.connection.request(
            "/os-keypairs/%s" % (key_pair.name), method="DELETE"
        )
    ...
    def ex_get_size(self, size_id):
        ...
        return self._to_size(
            self.connection.request("/flavors/%s" % (size_id,)).object["flavor"]
        )
    ...
    def _to_volume(self, api_node):
        ...
        state = self.VOLUME_STATE_MAP.get(
            api_node["status"], StorageVolumeState.UNKNOWN
        )
        ...
        return StorageVolume(
            ...
            driver=self,
            extra={
                "description": api_node.get(
                    "displayDescription", api_node.get("description")
                ),
                ...
                "location": api_node.get(
                    "availability_zone", api_node.get("availabilityZone")
                ),
    ...
    def _to_snapshot(self, data):
        ...
        display_name = data.get(
            "name", data.get("display_name", data.get("displayName", None))
        )
    ...
    def ex_create_floating_ip(self, floating_network_id, port_id=None):
        ...
        resp = self.connection.request(
            "/v2.0/floatingips", method="POST", data=data
        ).object
    ...
    def ex_associate_floating_ip_to_node(self, floating_ip_id, port_id):
        ...
        resp = self.connection.request(
            "/v2.0/floatingips/%s" % floating_ip_id, method="PUT", data=data
        ).object
...
class OpenStack_2_NodeDriver(OpenStack_1_1_NodeDriver):
    ...
    def get_image(self, image_id):
        ...
        """
        return self._to_image(
            self.image_connection.request("/v2/images/%s" % (image_id,)).object
        )
    ...
    def _to_network(self, obj):
        ...
        return OpenStackNetwork(
            id=obj["id"], name=obj["name"], cidr=None, driver=self, extra=extra
        )
    ...
    def create_volume_snapshot(
        ...
        return self._to_snapshot(
            self._get_volume_connection()
            .request("/snapshots", method="POST", data=data)
            .object
        )
    ...
    def _to_volume_type(self, data):
        ...
        name = data["name"]
        id_ = data.get("id", "")
        extra = {key: data[key] for key in data if key not in excluded_extra_values}
    ...
    def ex_list_compute_availability_zones_info(self):
        response = self.connection.request("/os-availability-zone").object
    ...
    def ex_list_storage_availability_zones_info(self):
        response = self._get_volume_connection().request("/os-availability-zone").object
    ...
    def ex_list_network_availability_zones_info(self):
        response = self.network_connection.request(
            "/v2.0/availability_zones", params={"resource": "network"}
        ).object
    ...
    def _to_availability_zones(self, data):
        return [self._to_availability_zone(key, **value) for key, value in data.items()]

    def _to_availability_zone(self, name, compute=False, storage=False, network=False):
        extra = {
            "compute": compute,
            "storage": storage,
            "network": network,
        }
        return NodeLocation(id=name, name=name, country=None, driver=self, extra=extra)
    ...
    def _to_floating_ip_pool(self, obj):
        return OpenStack_2_FloatingIpPool(
            obj["id"], obj["name"], self.network_connection
        )
    ...
    def ex_create_router(
        ...
        response = self.network_connection.request(
            "/v2.0/routers", method="POST", data=data
        ).object
...
class OpenStack_1_1_FloatingIpPool(object):
    ...
    def create_floating_ip(self):
        ...
        """
        resp = self.connection.request(
            "/os-floating-ips", method="POST", data={"pool": self.name}
        )
...
class OpenStack_1_1_FloatingIpAddress(object):
    ...
    def __repr__(self):
        return (
            "<OpenStack_1_1_FloatingIpAddress: id=%s, ip_addr=%s,"
            " network=%s, status=%s>"
            % (self.id, self.floating_ip_address, self.floating_network_id, self.status)
        )
