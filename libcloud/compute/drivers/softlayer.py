...
class SoftLayerNodeDriver(NodeDriver):
        ...
        if bare_metal:
            ...
            try:
                state = NODE_STATE_MAP.get(
                    str(host["hardwareStatusId"]), NodeState.UNKNOWN
                )
            except:
    ...
    def reboot_node(self, node):
        if node.extra.get("server_type", "") == "Bare Metal":
            self.connection.request("SoftLayer_Hardware", "rebootSoft", id=node.id)
        else:
    ...
    def create_node(
        ...
        if ex_backend_vlan:
            backend_network = {"networkVlan": {"id": int(ex_backend_vlan)}}
    ...
    def list_images(self, location=None):
        cloud_images = self.connection.request(
            "SoftLayer_Virtual_Guest", "getCreateObjectOptions"
        ).object
        cloud_images = [self._to_image(i) for i in cloud_images["operatingSystems"]]
        bm_images = self.connection.request(
            "SoftLayer_Hardware", "getCreateObjectOptions"
        ).object
        bm_images = [
            self._to_bare_metal_image(i) for i in bm_images["operatingSystems"]
        ]
    ...
    def list_sizes(self, location=None):
        bm_sizes = self.connection.request(
            "SoftLayer_Hardware", "getCreateObjectOptions"
        ).object
        bm_sizes = [
            self._to_bare_metal_size(size["preset"])
    ...
    def list_locations(self):
        locations = self.connection.request(
            "SoftLayer_Virtual_Guest", "getCreateObjectOptions"
        ).object
    ...
    def list_nodes(self):
        ...
        res = self.connection.request(
            "SoftLayer_Account", "getVirtualGuests", object_mask=virtual_mask
        ).object
        ...
        res = self.connection.request(
            "SoftLayer_Account", "getHardware", object_mask=bare_mask
        ).object
