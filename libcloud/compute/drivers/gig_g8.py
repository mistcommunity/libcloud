...
class G8NodeDriver(NodeDriver):
    ...
    def ex_expose_ssh_node(self, node, network=None):
        """
        ...
        :rtype: ``int``
        """
        network = network or node.extra["network"]
    ...
    def ex_destroy_network(self, network):
        ...
        self._api_request(
            "/cloudspaces/delete",
            {"cloudspaceId": int(network.id), "permanently": True},
        )
        return True
