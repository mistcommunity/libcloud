...
VM_SIZES = [
    ...
    {"id": "4 core, 8192Mb Ram", "memory": 8192, "cpu": 4},
    {"id": "8 core, 8192Mb Ram", "memory": 8192, "cpu": 8},
    {"id": "8 core, 10240Mb Ram", "memory": 10240, "cpu": 8},
]
...
cust_script_char_conv = (
    lambda script_str: script_str.replace(os.linesep, "&#13;")
    .replace('"', "&quot;")
    .replace("%", "&#37;")
    .replace("'", "&apos;")
)
...
cust_xml_char_conv_et_fix = (
    lambda xml_str: xml_str.replace("&amp;#13;", "&#13;")
    .replace("&amp;quot;", "&quot;")
    .replace("&amp;#37;", "&#37;")
    .replace("&amp;apos;", "&apos;")
)
...
class VCloudNodeDriver(NodeDriver):
    ...
    def reboot_node(self, node):
        res = self.connection.request(
            "%s/power/action/reset" % get_url_path(node.extra.get("href")),
            method="POST",
        )
    ...
    def ex_list_networks(self):
        ...
        for network in network_elements:
            ...
            try:
                network_object = self.connection.request(
                    get_url_path(network.get("href"))
                ).object
            except:
                continue
            ...
            nat_rules = network_object.findall(
                fixxpath(
                    network_object,
                    "Configuration/Features/NatService/" "NatRule/PortForwardingRule",
                )
            )
            rules = {}
            ...
            extra = {"href": network.get("href"), "nat_rules": rules}
    ...
    def list_images(self, location=None):
        ...
        app_images = self.connection.request(
            "/api/query?type=vAppTemplate&pageSize=500"
        ).object
...
class VCloud_1_5_NodeDriver(VCloudNodeDriver):
    ...
    def destroy_node(self, node, shutdown=True):
        ...
        res = self.connection.request(
            get_url_path(node.extra.get("href")), method="DELETE"
        )
        return res.status == httplib.ACCEPTED
    ...
    def ex_deploy_node(self, node):
        ...
        :rtype: :class:`Node`
        """
        data = {"powerOn": "true", "xmlns": "http://www.vmware.com/vcloud/v1.5"}
    ...
    def _change_vm_script(self, vapp_or_vm_id, vm_script, vm_script_text=None):
        ...
        for vm in vms:
            ...
            if (
                admin_auto_logon_enabled_elem is not None
                and admin_auto_logon_enabled_elem.text.strip() == "false"
            ):
    ...
    def _get_network_href(self, network_name):
        ...
        for l in links:
            if (
                l.attrib["type"] == "application/vnd.vmware.vcloud.orgNetwork+xml"
...
class VCloudNetwork(object):
    ...
    def __repr__(self):
        return '<VCloudNetwork id="%s" name="%s" cidr="%s">' % (
            self.id,
            self.name,
            self.cidr,
        )
