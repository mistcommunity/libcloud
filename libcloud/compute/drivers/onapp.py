...
RESOURCE_EXTRA_ATTRIBUTES_MAP = {
    "node": {
        ...
        "hypervisor_type": {"key_name": "hypervisor_type", "transform_func": str},
...
class OnAppNodeDriver(NodeDriver):
    ...
    def __init__(self, key=None, secret=None, host="onapp.com", port=443, verify=True):
        """
        :param key: username
        ...
        super(OnAppNodeDriver, self).__init__(
        ...
        try:
            ...
            raise Exception(
                "Make sure onapp host is accessible and port " "%s is open" % port
            )
    ...
    def create_node(
        ...
        ex_required_virtual_machine_startup=1,
        **kwargs,
    ):
        """
        Add a VS
        ...
        server_params = dict(
            ...
            required_virtual_machine_startup=ex_required_virtual_machine_startup,
    ...
    def create_key_pair(self, user, key):
        ...
        """
        action = "/users/{identifier}/ssh_keys.json".format(identifier=user)
    ...
    def ex_resize_node(self, node, **kwargs):
        ...
        response = self.connection.request(
            "/virtual_machines/{identifier}.json".format(identifier=node.id),
            data=data,
            headers={"Content-type": "application/json"},
            method="PUT",
        )
    ...
    def reboot_node(self, node):
        ...
        Reboot a node
        """
        action = "/virtual_machines/{identifier}/reboot.json".format(identifier=node.id)
    ...
    def list_nodes(self):
        ...
        nodes = [self._to_node(vm["virtual_machine"]) for vm in response.object]
        return nodes
    ...
    def list_locations(self):
        ...
        for location in locations.object:
            ...
            node_location = NodeLocation(
                id=l["id"], name=name, country=l["country"], extra=extra, driver=self
            )
    ...
    def _to_node(self, data):
        ...
        return Node(identifier, name, state, public_ips, private_ips, self, extra=extra)
    ...
    def _to_images(self, template_list):
        if template_list.get("hypervisor_group_id"):
            ...
            images = [
                self._to_image(image, hypervisor_group_id=h)
    ...
    def _to_image(self, image, hypervisor_group_id=None):
        ...
        extra["min_disk_size"] = image["image_template"].get("min_disk_size")
        extra["min_memory_size"] = image["image_template"].get("min_memory_size")
        extra["virtualization"] = image["image_template"].get("virtualization")
        image_name = image["image_template"].get("label")
        image_id = image["image_template"].get("id")
        return NodeImage(id=image_id, name=image_name, driver=self, extra=extra)

    def _to_network(self, network):
        extra = {}
        extra["location_group_id"] = network["location_group_id"]
        return Network(
            id=network["id"], name=network["label"], cidr_block=None, extra=extra
        )
