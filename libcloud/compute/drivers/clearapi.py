...
class ClearAPINodeDriver(NodeDriver):
    ...
    NODE_STATE_MAP = {"Active": NodeState.RUNNING, "off": NodeState.OFF}
    ...
    def list_nodes(self):
        ...
        response = self.connection.request(
            "/clearos/clearapi/v2/rest/host/get_all_host"
        )
        nodes = [self._to_node(host) for host in response.object["data"]]
        return nodes
    ...
    def ex_start_node(self, node):
        ...
        res = self.connection.request(
            "/clearos/clearapi/v2/rest/host/power/on",
    ...
    def ex_stop_node(self, node):
        data = {"uuid": node.extra["uuid"]}
        res = self.connection.request(
            "/clearos/clearapi/v2/rest/host/power/off",
            data=json.dumps(data),
            method="POST",
        )
        return res.status in [httplib.OK, httplib.CREATED, httplib.ACCEPTED]
    ...
    def ex_get_host(self, node):
        data = {"uuid": node.extra["uuid"]}
        response = self.connection.request(
            "/clearos/clearapi/v2/rest/host/get_host",
            data=json.dumps(data),
            method="POST",
        )
