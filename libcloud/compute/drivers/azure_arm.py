...
class AzureNodeDriver(NodeDriver):
    ...
    def create_node(
        ...
        for disk in ex_data_disks:
            ...
            if disk.get("id"):  # existing disk
                ...
                new_disk = {
                    ...
                    "caching": caching,
                    "lun": lun,
                    "managedDisk": {"storageAccountType": disk_type},
                }
        ...
        r = self.connection.request(
            target, params={"api-version": "2018-04-01"}, data=data, method="PUT"
        )
    ...
    def reboot_node(self, node):
        ...
        try:
            self.connection.request(
                target, params={"api-version": RESOURCE_API_VERSION}, method="POST"
            )
            return True
    ...
    def destroy_node(
        ...
        try:
            self.connection.request(
                node.extra["id"], params={"api-version": "2015-06-15"}, method="DELETE"
            )
        except BaseHTTPError as h:
            if h.code == 202:
                pass
        ...
        while do_node_polling and retries > 0:
            try:
                ...
                self.connection.request(
                    node.extra["id"], params={"api-version": RESOURCE_API_VERSION}
                )
                retries -= 1
            except BaseHTTPError as h:
    ...
    def list_volumes(self, ex_resource_group=None):
        ...
        response = self.connection.request(
            action, method="GET", params={"api-version": RESOURCE_API_VERSION}
        )
    ...
    def ex_create_resource_group(self, name, location):
        ...
        data = {"name": name, "location": location_id}
        action = "/subscriptions/%s/resourceGroups/%s/" % (self.subscription_id, name)
        r = self.connection.request(
            action, params={"api-version": "2017-08-01"}, data=data, method="PUT"
        )
    ...
    def ex_get_resource_group(self, name):
        ...
        return AzureResourceGroup(
            resp["id"], resp["name"], resp["location"], resp["properties"]
        )
    ...
    def ex_resource_group_exists(self, name):
        ...
        try:
            self.connection.request(
                action, params={"api-version": "2021-04-01"}, method="HEAD"
            )
    ...
    def ex_list_resource_groups(self):
        ...
        r = self.connection.request(action, params={"api-version": "2016-09-01"})
        return [
            AzureResourceGroup(
    ...
    def ex_create_storage_account(self, name, ex_resource_group, kind, location):
        ...
        ret = self.connection.request(
            action, params={"api-version": "2017-06-01"}, data=data, method="PUT"
        )
    ...
    def ex_list_storage_accounts(self):
        ...
        r = self.connection.request(action, params={"api-version": "2017-06-01"})
        return [
            AzureStorageAccount(
                grp["id"], grp["name"], grp["location"], grp["properties"]
            )
    ...
    def ex_get_storage_account(self, name, resource_group):
        ...
        resp = self.connection.request(
            action, params={"api-version": "2021-04-01"}
        ).object
    ...
    def ex_check_storage_account_name(self, name):
        ...
        data = {"name": name, "type": "Microsoft.Storage/storageAccounts"}
        resp = self.connection.request(
            action, params={"api-version": "2021-04-01"}, data=data, method="POST"
        ).object
    ...
    def ex_create_network_security_group(
        self, name, resource_group, location=None, securityRules=[]
    ):
        """
        ...
        r = self.connection.request(
            target, params={"api-version": "2017-09-01"}, data=data, method="PUT"
        )
    ...
    def ex_delete_network_security_group(self, name, resource_group, location=None):
        ...
        self.connection.request(
            target, params={"api-version": "2016-09-01"}, data=data, method="DELETE"
        )
    ...
    def ex_get_network(self, network_name, resource_group):
        ...
        net = self.connection.request(
            action, params={"api-version": "2020-11-01"}
        ).object

        return AzureNetwork(net["id"], net["name"], net["location"], net["properties"])
    ...
    def ex_update_nic_properties(self, network_interface, resource_group, properties):
        ...
        r = self.connection.request(
            target, params={"api-version": "2018-06-01"}, data=data, method="PUT"
        )
        return AzureNic(
            r.object["id"],
            r.object["name"],
    ...
    def ex_destroy_nic(self, nic):
        ...
        try:
            self.connection.request(
                nic.id, params={"api-version": "2015-06-15"}, method="DELETE"
            )
            return True
        except BaseHTTPError as h:
    ...
    def ex_get_node(self, id):
        ...
        r = self.connection.request(id, params={"api-version": RESOURCE_API_VERSION})
    ...
    def ex_create_public_ip(
        ...
        r = self.connection.request(
            target, params={"api-version": "2015-06-15"}, data=data, method="PUT"
        )
        return self._to_ip_address(r.object)
    ...
    def ex_create_tags(self, resource, tags, replace=False):
        ...
        if not isinstance(resource, basestring):
            resource = resource.id
        r = self.connection.request(resource, params={"api-version": "2018-06-01"})
        if replace:
            r.object["tags"] = tags
        else:
    ...
    def start_node(self, node):
        ...
        r = self.connection.request(
            target, params={"api-version": "2015-06-15"}, method="POST"
        )
        return r.object
        ...
        try:
            ...
            for status in r.object["statuses"]:
                if status["code"] in ["ProvisioningState/creating"]:
                    ...
                    created_at = self._find_earlier_date(created_at, status.get("time"))
            ...
            if "osDisk" in disk.get("name", ""):
                for status in disk.get("statuses", []):
                    if status["code"] in [
                        ...
                        created_at = self._find_earlier_date(
    ...
    def _to_node(self, data, fetch_nic=True, fetch_power_state=True):
        ...
        try:
            extra["storageUri"] = data["properties"]["diagnosticsProfile"][
                "bootDiagnostics"
            ]["storageUri"]
        except:
            pass
        ...
        for datadisk in extra["storageProfile"]["dataDisks"]:
            try:
                extra["disk-%s" % datadisk["name"]] = "%sGB - %s" % (
                    datadisk["diskSizeGB"],
                    datadisk["vhd"]["uri"],
                )
            except:
        ...
        extra["networkProfile"] = data["properties"]["networkProfile"][
            "networkInterfaces"
        ]
        extra["subnet"] = subnet
    ...
    def _get_instance_vhd(
        ...
        while n < 10:
            try:
                instance_vhd = "https://%s.blob%s" "/%s/%s.vhd" % (
                    ex_storage_account,
