...
class ClearCenterNodeDriver(NodeDriver):
    ...
    def __init__(self, key=None, uri="https://api.clearsdn.com", verify=True):
        """
        :param key: apikey
        ...
        super(ClearCenterNodeDriver, self).__init__(key=key, uri=uri, secure=secure)
        ...
        try:
            ...
            raise Exception(
                "Make sure clearcenter host <%s> is accessible and port "
                "%s is open" % (host, port)
            )
    ...
    def _to_node(self, data):
        ...
        return Node(id, name, state, public_ips, private_ips, self, extra=extra)
    ...
    def _get_extra_dict(data):
        ...
        extra["software_name"] = (
            data["software"].get("release", "")
            + " "
            + data["software"].get("versionn", "")
        )
    ...
    def list_nodes(self):
        ...
        nodes = [self._to_node(device) for device in response.object["data"]]
        return nodes
