...
class CloudFilesStorageDriver(StorageDriver, OpenStackDriverMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(CloudFilesStorageDriver, self).__init__(
            key=key,
            secret=secret,
            secure=secure,
            host=host,
    ...
    def get_object(self, container_name, object_name):
        ...
        response = self.connection.request(
            "/%s/%s" % (container_name_encoded, object_name_encoded), method="HEAD"
        )
    ...
    def get_container_cdn_url(self, container, ex_ssl_uri=False):
        ...
        response = self.connection.request(
            "/%s" % (container_name_encoded), method="HEAD", cdn_request=True
        )
    ...
    def enable_container_cdn(self, container, ex_ttl=None):
        ...
        # pylint: disable=unexpected-keyword-arg
        response = self.connection.request(
            "/%s" % (container_name), method="PUT", headers=headers, cdn_request=True
        )
    ...
    def delete_container(self, container):
        ...
        if response.status == httplib.NO_CONTENT:
            ...
            raise ContainerDoesNotExistError(value="", container_name=name, driver=self)
        elif response.status == httplib.CONFLICT:
            # @TODO: Add "delete_all_objects" parameter?
            raise ContainerIsNotEmptyError(value="", container_name=name, driver=self)
    ...
    def download_object(
        ...
        response = self.connection.request(
            "/%s/%s" % (container_name, object_name), method="GET", raw=True
        )
    ...
    def download_object_range(
        ...
        response = self.connection.request(
            "/%s/%s" % (container_name, object_name),
            method="GET",
            headers=headers,
            raw=True,
        )
    ...
    def ex_multipart_upload_object(
        self,
        file_path,
        container,
        object_name,
        ...
        return self._upload_object_manifest(
            container=container,
            object_name=object_name,
            extra=extra,
            verify_hash=verify_hash,
    ...
    def ex_enable_static_website(self, container, index_file="index.html"):
        ...
        # pylint: disable=unexpected-keyword-arg
        response = self.connection.request(
            "/%s" % (container_name), method="POST", headers=headers, cdn_request=False
        )
    ...
    def ex_get_object_temp_url(self, obj, method="GET", timeout=60):
        ...
        path = "%s/%s/%s" % (self.connection.request_path, obj.container.name, obj.name)
        try:
    ...
    def _upload_object_manifest(
        ...
        data = ""
        response = self.connection.request(
            request_path, method="PUT", data=data, headers=headers, raw=True
        )
    ...
    def _put_object(
        ...
        if response.status == httplib.EXPECTATION_FAILED:
            raise LibcloudError(value="Missing content-type header", driver=self)
            ...
            raise LibcloudError("status_code=%s" % (response.status), driver=self)
    ...
    def _encode_container_name(self, name):
        ...
        if name.find("/") != -1:
            raise InvalidContainerNameError(
                value="Container name cannot" " contain slashes",
                container_name=name,
                driver=self,
            )
    ...
    def _to_container_list(self, response):
        ...
        for container in response:
            extra = {
                "object_count": int(container["count"]),
