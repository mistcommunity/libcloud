...
class AuroraDNSDriverTests(LibcloudTestCase):
    ...
    def test_create_record(self):
        ...
        record = zone.create_record(
            name="localhost", type=RecordType.A, data="127.0.0.1", extra={"ttl": 900}
        )
    ...
    def test_delete_zone_non_exist(self):
        try:
            self.driver.delete_zone(
                Zone(
                    id=1,
                    domain="nonexists.example.com",
                    type="NATIVE",
                    driver=AuroraDNSDriver,
                    ttl=3600,
                )
            )
            self.fail("expected a ZoneDoesNotExistError")
    ...
    def test_create_health_check(self):
        ...
        check = self.driver.ex_create_healthcheck(
            zone=zone,
            type=type,
            hostname=hostname,
            port=port,
...
class AuroraDNSDriverMockHttp(MockHttp):
    ...
    def _zones_ffb62570_8414_4578_a346_526b44e320b7_records(
    ...
    def _zones_ffb62570_8414_4578_a346_526b44e320b7_health_checks(
        self, method, url, body, headers
    ):
        if method == "POST":
    ...
    def _zones_ffb62570_8414_4578_a346_526b44e320b7_records_5592f1ff(
        self, method, url, body, headers
    ):
        body = None
        if method == "GET":
